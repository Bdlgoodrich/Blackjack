...
public class Player extends Utils {
    ...
    private boolean getPlay(Hand hand, Bet bet) {
        while (true) {
            ...
            if (response.contains("double") || response.contains("down") || response.equals("d") || response.contains("3")) {
            ...
            } else if (response.contains("split") || response.contains("4")) {
            ...
            } else if (response.contains("hit") || response.equals("h") || response.contains("1")) {
                hand.hit();
                hand.showHand();
                hand.showScore();
                if (getScore() == -1) break;
            } else if (response.contains("stand") || response.equals("s") || response.contains("2")) {
                hand.stand();
                break;
    ...
    private void showOptions(Hand hand, Bet bet) {
        ...
        showText("1. Hit(h)", TextColors.BLUE);
        showText("2. Stand(s)", TextColors.BLUE);

        //if they have not yet hit or split
        if (hand.getCardCount() == 2 && !isSplit) {
            //if they have enough money, offer to double down
            if (bet.verifyBankrollTwiceBet()) {
                showText("3. Double Down(d)", TextColors.BLUE);
    ...
    public boolean offerNewHand() {
        if (bet.getBankRoll() >= bet.getTableMinimum()) {
            ...
            bet.resetBet();
            showText("You do not have enough money to continue at this table." +
                    "Would you like to move to a different one?\n1. Yes(y)\n2. No(n)");
            String response = getStringResponse();
            if (containsNegative(response)) {
                showText("Thank you for playing. Good bye.");
                return false;
